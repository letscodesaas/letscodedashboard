...
export const PATCH = async (req:NextRequest, { params }:{ params: { id: string } }) => {
    ...
    try {
        ...
        if (!updated) {
            return NextResponse.json({ message: 'Failed to toggle feature status' }, { status: 500 });
        }
        ...
        return NextResponse.json({ message: 'Failed to toggle feature status', error }, { status: 500 });
    }
}