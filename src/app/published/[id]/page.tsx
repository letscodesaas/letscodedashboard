...
function NewsletterPage() {
  ...
  if (!info.title && !info.content) {
    return (
      <div className="flex h-screen items-center justify-center bg-gray-50 w-full">
        <div className="rounded-lg bg-white p-8 shadow-md">
          <h2 className="mb-4 text-xl font-semibold text-gray-800">Newsletter Not Found</h2>
          <p className="text-gray-700">The newsletter youre looking for doesn&apos;t exist or has been removed.</p>
        </div>
      </div>
    );
  }
  ...
  // Format date
  const formattedDate = info.createdAt ? new Date(info.createdAt).toLocaleDateString('en-US', {
    year: 'numeric',
    month: 'long',
    day: 'numeric',
  }) : 'Date not available';
    ...
    <div className="h-screen bg-gray-50 py-8 w-full overflow-auto">
      <div className="mx-auto max-w-4xl rounded-lg bg-white p-6 shadow-md">
        ...
        <div className="mt-8 border-t pt-4 text-center text-sm text-gray-500">
          ...
          <p className="mt-1">© {new Date().getFullYear()} Let&apos;s Code. All rights reserved.</p>
        </div>
      </div>
    </div>
  );
