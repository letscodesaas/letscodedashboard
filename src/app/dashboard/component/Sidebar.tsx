    ...
    const getConditionalItems = () => {
        ...
        if (role === "admin") {
            items.push(
                ...
                { title: "Create Jobs", url: "/dashboard/jobs/create", icon: Briefcase },
                { title: "Show All Jobs", url: "/dashboard/jobs/show", icon: Briefcase },
                { title: "Create Products", url: "/dashboard/product/create", icon: Package },
                { title: "Show Products", url: "/dashboard/product/show", icon: Package },
        ...
        if (policy?.[2]?.access) {
            if (policy[2].resources.includes("Manage")) {
                items.push({ title: "Manage Teams", url: "/dashboard/team/manage", icon: Users });
            }
            if (policy[2].resources.includes("Create")) {
                items.push({ title: "Create Teams", url: "/dashboard/team/create", icon: Users });
            }
        }

        if (policy?.[0]?.access) {
            if (policy[0].resources.includes("Create")) {
                items.push({ title: "Create Jobs", url: "/dashboard/jobs/create", icon: Briefcase });
            }
            if (policy[0].resources.includes("Manage")) {
                items.push({ title: "Show All Jobs", url: "/dashboard/jobs/show", icon: Briefcase });
            }
        }

        if (policy?.[1]?.access) {
            if (policy[1].resources.includes("Create")) {
                items.push({ title: "Create Products", url: "/dashboard/product/create", icon: Package });
            }
            if (policy[1].resources.includes("Manage")) {
                items.push({ title: "Show Products", url: "/dashboard/product/show", icon: Package });
            }
        }

        if (policy?.[3]?.access) {
            if (policy[3].resources.includes("Create")) {
                items.push({ title: "Newsletter", url: "/dashboard/newsletter", icon: Mail });
            }
            if (policy[3].resources.includes("Manage")) {
                items.push({ title: "Show All Newsletters", url: "/published", icon: Mail });
            }
        }
                        ...
                        <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                                ...
                                >
                                    ...
                                    <div className="grid flex-1 text-left text-sm leading-tight">
                                        <span className="truncate font-semibold">{userInitials}</span>
