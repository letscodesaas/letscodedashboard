...
export default function Question({ id, type }: { id: string; type: string }) {
  ...
  useEffect(() => {
    (async function fetchQuestion() {
      try {
        const response = await trpc.question.getquestion.mutate({
          id: id,
        });
        ...
        setExpectedInputs(response.data.exceptedInput);
  ...
  const onSubmit = async (e: React.FormEvent) => {
    try {
      e.preventDefault();
      setLoading(true);
      const response = await trpc.question.updatequestion.mutate({
        id: id,
        title,
        content,
        categories: category,
      ...
      <Card className="w-full max-w-7xl shadow-lg rounded-2xl">
        <CardHeader>
          <CardTitle className="text-2xl font-semibold">
            {type === 'view' ? 'View Questions' : 'Update Questions'}
          </CardTitle>
        </CardHeader>
        <CardContent>
          <form
