...
export default function AIToolsStats() {
  ...
  const toolChartData = stats.toolUsageByType.map((item: any) => ({
    name: TOOL_NAMES[item._id] || item._id,
    value: item.count,
    success: item.successCount,
  }));

  const successRateData = stats.successRateByTool.map((item: any) => ({
    name: TOOL_NAMES[item.tool] || item.tool,
    rate: item.successRate,
    total: item.total,
  }));

  const trendData = stats.usageTrends.map((item: any) => ({
    date: item._id,
    usage: item.count,
    success: item.successCount,
  }));

  const actionData = stats.actionBreakdown.map((item: any) => ({
    action: item._id,
    count: item.count,
    success: item.successCount,
  }));
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-gray-500 mt-1">{overall.uniqueUsers} unique users</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-3xl font-bold text-green-600">{overall.successRate}%</div>
            <p className="text-xs text-gray-500 mt-1">{overall.successCount} successful</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-3xl font-bold">{overall.avgResponseTime}ms</div>
            <p className="text-xs text-gray-500 mt-1">Max: {overall.maxResponseTime}ms</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-3xl font-bold text-red-600">{overall.failureCount}</div>
            <p className="text-xs text-gray-500 mt-1">{((overall.failureCount / overall.totalUsage) * 100).toFixed(2)}% failure rate</p>
          </CardContent>
        </Card>
      </div>
        ...
        <CardContent>
          <ResponsiveContainer width="100%" height={300}>
            <LineChart data={trendData}>
              ...
              <Tooltip />
              <Legend />
              <Line type="monotone" dataKey="usage" stroke="#3b82f6" name="Total Usage" strokeWidth={2} />
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-4">
              {stats.toolUsageByType.map((tool: any) => (
                <div key={tool._id} className="flex items-center justify-between border-b pb-3">
                  <div>
                    <p className="font-medium">{TOOL_NAMES[tool._id] || tool._id}</p>
                    <p className="text-sm text-gray-500">{tool.count} total uses</p>
                  </div>
                  <div className="text-right">
                    <p className="font-bold text-lg">{((tool.successCount / tool.count) * 100).toFixed(1)}%</p>
                    <p className="text-sm text-gray-500">Avg: {tool.avgResponseTime.toFixed(0)}ms</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b">
                  ...
                  <th className="text-left py-2 px-4 font-semibold">Total Count</th>
                  <th className="text-left py-2 px-4 font-semibold">Success</th>
                  <th className="text-left py-2 px-4 font-semibold">Success Rate</th>
                </tr>
              </thead>
              <tbody>
                {actionData.map((action: any) => (
                  <tr key={action.action} className="border-b hover:bg-gray-50">
                    ...
                    <td className="py-2 px-4">
                      <span className={`font-semibold ${((action.success / action.count) * 100) >= 90 ? 'text-green-600' : 'text-orange-600'}`}>
